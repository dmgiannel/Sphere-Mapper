...
struct latLongUp getLLInfo(struct vertex cpos, struct vertex cup){
	double x = atan2(cpos.z, cpos.x)*180/M_PI;
	//printf ("long: %f, ", cpos.x);
	double y = (acos(cpos.y/sqrt(cpos.x*cpos.x+cpos.y*cpos.y+cpos.z*cpos.z)) - M_PI)*180/M_PI;
	//printf("lat: %f\n", cpos.z);
	struct latLongUp llu;
	llu.lat = y;
	llu.longi = x;
	llu.up = 0;
	return llu;
